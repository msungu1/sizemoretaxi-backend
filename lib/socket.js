...
// ✅ ALWAYS notify admin of this driver's location, trip or no trip
        const driverDoc = await User.findById(driverId).select("name");
...
//         // ✅ Only look up the trip if tripId is a real, valid ObjectId
//         if (data.tripId && mongoose.Types.ObjectId.isValid(data.tripId)) {
//             try {
//                 trip = await Trip.findById(data.tripId)
//                     .populate("rider")
//                     .populate("driver");
//             } catch (lookupErr) {
//                 console.log("⚠️ Trip lookup failed, continuing without trip data:", lookupErr.message);
//             }
//         } else {
//             console.log("⚠️ No valid tripId provided, sending alert without trip context");
//         }
...
//         // ✅ This now always runs, trip or no trip
//         io.to("admin_room").emit("admin_emergency_alert", emergencyPayload);

//         console.log("🚨 Emergency alert sent to admin_room");

//     } catch (error) {
//         console.log("❌ emergency_alert error:", error);
//     }
// });  
    ...
    try {
        ...
        if (data.tripId && mongoose.Types.ObjectId.isValid(data.tripId)) {
            try {
                ...
                console.log("⚠️ Trip lookup failed:", lookupErr.message);
            }
        }

        let riderInfo = trip?.rider
            ? {
                id: trip.rider._id,
                name: trip.rider.name,
                phone: trip.rider.phone,
        ...
        // ✅ FALLBACK: no valid trip, but we know who's connected via the socket itself
        if (!riderInfo && !driverInfo && userId && mongoose.Types.ObjectId.isValid(userId)) {
            const triggeringUser = await User.findById(userId);
            if (triggeringUser) {
                const userInfo = {
                    id: triggeringUser._id,
                    name: triggeringUser.name,
                    phone: triggeringUser.phone,
                    carModel: triggeringUser.carModel,
                    carNumber: triggeringUser.carNumber,
                };
                if (data.triggeredBy === "driver") {
                    driverInfo = userInfo;
                } else {
                    riderInfo = userInfo;
                }
